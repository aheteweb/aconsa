...
//Global vars 
	var altPage;//Will store the content of alternative language
	var pageLang = $('body').attr('data-lang');
	var altPath = pageLang === 'en' ? 'es/' + $('body').attr('data-name') : $('body').attr('data-name');
	var gOwner = 'aheteweb';//Repo owner
	var gRepo = 'aconsa';//Repo name
	var imagesUrl = 'https://sleepy-tundra-28817.herokuapp.com';
...
//Init dropzone
	function initDropZone(){
		...
		var myDropzone = new Dropzone('#dropzone-area', {
			...
			url: 'https://api.cloudinary.com/v1_1/aconsa/image/upload'
			//url: '#'
		});
		...
		myDropzone.on('sending', function (file, xhr, formData) {
			...
			formData.append('api_key', '318448675986743');
			formData.append('timestamp', Date.now() / 1000 | 0);
			formData.append('upload_preset', 'unsbsfhj');
				...
				//Apply js to new elements
					...
					//Page title
						if(section === 'page_title'){
							var pageName = $('body').attr('data-name').replace('.html', '');
							if(pageLang === 'en'){
								var str = mH.pages[pageName].title_en;
								var res = str.toUpperCase();
								$('#mhReceived .js-content h1').text(res);
							}else{
								var str = mH.pages[pageName].title_es;
								var res = str.toUpperCase();								
								$('#mhReceived .js-content h1').text(res);
							}
						}
...
//Admin page
	...
	//General Options (_config.yml)
		function populateConfig (){
			...
			$('#title-en').val(content.title.en);
			$('#title-es').val(content.title.es);
			$('#desc-en').val(content.description.en);
			$('#desc-es').val(content.description.es);
	...
	//Manage internal pages
		function populateInternalPages(section){
			...
			$.each(mH.pages, function(key, pair){
				$('#mhAdminpage .list-pages').append('<li class="list-group-item">' + pair.title_en + '<div class="btn-group btn-group-sm float-right"><button type="button" class="btn xs btn-outline-secondary view" data-url="' + pair.permalink_en + '"><i class="fas fa-eye"></i></button><button type="button" class="btn xs btn-outline-danger rm" data-page="' + key + '"><i class="fas fa-times-circle"></i></button></div></li>');
			});
		}
		...
		$(document).on('click', '.save-page', function(){
			if(!$('#pagetitle-en').val() || !$('#pagetitle-es').val()){
				alert('Debes completar ambos campos');
			}else{
				loading('body');
				var enTitle = $('#pagetitle-en').val();
				var enPath = formatThis(enTitle, 'url');
				var esTitle = $('#pagetitle-es').val();
				var esPath = formatThis(esTitle, 'url');

				var enContent  = '---\n';
						enContent += 'layout: default\n';
						enContent += 'title: ' + enTitle + '\n';
						enContent += 'alttitle: ' + esTitle + '\n';
						enContent += 'permalink: ' + enPath + '\n';
						enContent += 'altpermalink: /es/' + esPath + '\n';
						enContent += 'lang: en\n';
						enContent += '---\n';
						
				var esContent  = '---\n';
						esContent += 'layout: default\n';
						esContent += 'title: ' + esTitle + '\n';
						esContent += 'alttitle: ' + enTitle + '\n';
						esContent += 'permalink: ' + esPath + '\n';
						esContent += 'altpermalink: /en/' + enPath + '\n';
						esContent += 'lang: es\n';
						esContent += '---\n';
	...
	//Edit Navigation
		function populateNavigation(){
			...
			$.each(mH.pages, function(i, v){
				$('#mhAdminpage .all-pages').append('<li class="list-group-item" data-permalink_en="' + v.permalink_en + '" data-permalink_es="' + v.permalink_es + '" data-title_en="' + v.title_en + '" data-title_es="' + v.title_es + '">' + v.title_en + '<div class="btn-group btn-group-sm float-right"><span class="btn xs btn-outline-secondary move"><i class="fas fa-arrows-alt"></i></span><span class="btn xs btn-outline-danger rm"><i class="fas fa-times-circle"></i></span></div></li>');
			});			
...
//Start-Stop-Save
		function edit(){
			...
			if(altPage === undefined && $('#mhContent > [data-mhsection]').length !== 0){
				getContents({
					owner: gOwner,
					repo: gRepo,
					path: altPath,
					action: function(data, status, xhr){
						altPage = $.trim(decodeContent(atob(data.content)).split('---')[2]);
						if(altPage){
							$('#mhContent [data-editable]').each(function(i,val){
								if(pageLang === 'en'){// Get the traduced content and append it to the original
									var toFind = parseInt($(val).attr('data-name')) - 1;
								}else{
									var toFind = parseInt($(val).attr('data-name')) + 1;
								}
								var toAdd = $(altPage).find('[data-name=' + toFind + ']');
								$(val).after(toAdd[0].outerHTML);
							});	
						}
						initSlider('.slider');
						altPage;
...
//Edit section's content
	...
	function editSection(section){
		...
		//Clean the selected language
		$('#mhSectionOptions .edit-language a').removeClass('active');
		$('.preview').empty().attr('data-target', pageLang);
		//activate the tab for the current page language
		$('#mhSectionOptions ul [data-target=' + pageLang + ']').addClass('active');				
	...
	//Select the language to edit
		$(document).on('click', '#mhSectionOptions .edit-language a', function(){
			$('#mhSectionOptions .preview').attr('data-target', $(this).attr('data-target'));
			$('#mhSectionOptions .edit-language a').toggleClass('active');
		});	
	...
	var editFunctions = {
		header_slider: function(){
			...
			var bgImg = $('#mhSectionOptions .slider-edit .slide:first-of-type').css('background-image').replace('url("', '').replace('")', '');
		...
		slider: function(){
			...
			destroySlider($(slider));
			//Append the slider raw html to the preview
			$('.preview .slider-edit').addClass('no-text').append($(editingSection).clone());
			//Add the active class (so I can edit one slide at a time)
			$('#mhSectionOptions .slider-edit .slide:first-of-type').addClass('active');
			//get the slide image and place it like the input value
			var bgImg = $('#mhSectionOptions .slider-edit .slide:first-of-type').css('background-image').replace('url("', '').replace('")', '');
		...
		//Slider
			//navigate the slides
				function changeSlide(action){
					...
					var bgImg = $('.slider-edit .slide.active').css('background-image').replace('url("', '').replace('")', '');
					var imgName = bgImg.lastIndexOf('/');
					imgName = bgImg.substring(imgName + 1);
					if(!bgImg.includes("picsum")){
			...
			//Add a slide
				$(document).on('click', '.slider-edit .add', function(){
					...
					if($('.preview .slider-edit').hasClass('no-text')){
						...
						$('.slider-edit .slider').append('<div class="slide active" style="background-image: url(https://picsum.photos/1200/600/?random)"><div class="slide-content"><div class="content"><div data-editable data-lang="es" data-name="' + timestampEs + '"><h2>Edita este texto</h2></div><div data-editable data-lang="en" data-name="' + timestampEn + '"><h2>Edit this text</h2></div></div></div></div>');
					}
					editorInit('#mhSectionOptions', true);
					currentSlide();
	...
	//Session
		...
		function changePass(){
			...
			var mail = user.email;
			var pass = $.trim($('#current-pass').val());
			var newpass = $.trim($('#new-pass').val());
			var confirmpass = $.trim($('#confirm-pass').val());			
			var credential = firebase.auth.EmailAuthProvider.credential(mail, pass);
			if(pass && newpass && confirmpass && newpass === confirmpass){
				console.log('1')
				$('.btn.change').attr("disabled", true).text('Please wait...');
				user.reauthenticateWithCredential(credential).then(function() {
					updatePass({
						pass: newpass,
						success: function(){
							$('.success-alert').toggle();
							$('.btn.change').attr("disabled", false).text('Change password');
						},
						error: function(error){
							$('.change-alert').toggle().text(error.message);
							$('.btn.change').attr("disabled", false).text('Change password');
						}
					})					
				...
				console.log('2')
				if(!pass){
					$('.current-alert').toggle()
				}
				if(!newpass){
					$('.new-alert').toggle();
				}
				if(!confirmpass){
					$('.confirm-alert').toggle().text('Field can\'t be empty.');
				}
				if(newpass !== confirmpass){
					$('.confirm-alert').toggle().text('Passwords don\'t match.');
				}				
			}
		}
